...
def record_interaction_pending(
    ...
    cursor.execute(
        """
        INSERT INTO dbo.InteraccionesRAG (ConversacionId, Pregunta, Respuesta, Fuentes, Contexto, Estado, Confianza)
        OUTPUT INSERTED.Id
        VALUES (?, ?, ?, ?, ?, 'pendiente', ?)
        """,
        conversation_id,
...
def list_pending_interactions(limit: int = 50) -> List[Dict]:
    ...
    cursor.execute(
        """
        SELECT TOP (?) Id, ConversacionId, Pregunta, Respuesta, Fuentes, FechaCreacion, Confianza
